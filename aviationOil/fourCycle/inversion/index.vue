  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="作业任务" align="center">
        <template slot-scope="scope">
          {{ scope.row.transferType | showType }}
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogOilVisible" title="库内油罐倒库" width="1260px" height="50%" @close="cancleForm">
      <el-form :inline="true" ref="billForm" label-position="right" :rules="billRules" :model="billData" label-width="100px" size="small">
        <el-row>
          <el-col :span="24">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="作业任务" prop="transferType" :inline-message="true">
              <span v-model="billData.transferType">倒罐</span>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    <el-dialog title="油罐倒库详情" :visible.sync="tableVisible" width="1000px">
      <el-row class="info">
        <el-col :span="6">日期：<span>{{ info.createTime }}</span></el-col>
        <el-col :span="6">发给：<span>{{ info.transferTo }}</span></el-col>
        <el-col :span="6">流水号：<span>{{ info.serialNumber }}</span></el-col>
        <el-col :span="6">指挥员：<span>{{ info.commander }}</span></el-col>
      </el-row>
      <el-row class="info">
        <el-col :span="12">
          <span>作业任务：</span>{{info.transferType | showType}}
        </el-col>
        <el-col :span="12">
          <span>工艺流程：</span>{{info.transferProcess}}
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
          <span>作业纪要：</span>{{info.jobSummary}}
        </el-col>
      </el-row>
      <el-table
        :data="tableList"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      operation.homeworkList(this.listQuery).then((r) => {
        if (r.code === 20000) {
    ...
    submitForm() {
      ...
      if((this.billData.transferProcess1.length===0)&&(this.billData.transferProcess2.length===0)&&(this.billData.transferProcess3.length===0)){
        this.processFlag = false
      }else{
        if(this.billData.transferProcess1){
          this.billData.transferProcess1 = this.billData.transferProcess1.toString().split(',').join('-')
        }
        if(this.billData.transferProcess2){
          this.billData.transferProcess2 = this.billData.transferProcess2.toString().split(',').join('-')
        }
        if(this.billData.transferProcess3){
          this.billData.transferProcess3 = this.billData.transferProcess3.toString().split(',').join('-')
        }
      ...
      if (this.infoFlag === false) {
        ...
        operation.homework(formParams).then((r) => {
          if (r.code === 20000) {
    ...
    showDetail(row) {
      ...
      this.info = row
      this.info.transferProcess = this.info.transferProcess.split(',').join('→')
    },
    // 加载详情列表
    getTableList(row) {
      operation.homeworkDetail({
