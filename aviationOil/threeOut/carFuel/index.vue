<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button type="primary" icon="el-icon-s-order" @click="planeFuel">加油</el-button>
    ...
    <div v-if="inFlag">
      ...
      >
        ...
        <el-table-column label="密度" align="center">
          <template slot-scope="scope">{{ scope.row.oilDensity }}</template>
        </el-table-column>
        <el-table-column label="体积 (L)" align="center">
          <template slot-scope="scope">{{ scope.row.oilVolume }}</template>
        </el-table-column>
        <el-table-column label="重量 (kg)" align="center">
          <template slot-scope="scope">{{ scope.row.oilWeight }}</template>
        </el-table-column>
        <el-table-column label="车辆" align="center">
          <template slot-scope="scope">{{ scope.row.tankName }}</template>
        </el-table-column>
    ...
    <div v-if="outFlag">
      ...
      >
        ...
        <el-table-column label="经办人" align="center">
          <template slot-scope="scope">{{ scope.row.otherPerson }}</template>
        </el-table-column>
        <el-table-column label="密度" align="center">
          <template slot-scope="scope">{{ scope.row.oilDensity }}</template>
        </el-table-column>
        ...
        <el-row>
          <el-col :span="24">
            ...
            <el-form-item label="飞机" prop="aircraftId">
              <el-select clearable placeholder="请选择" v-model="billData.aircraftId">
                <el-option
                  v-for="item in planeOption"
                  :key="item.id"
                  :label="item.fighterName"
...
export default {
  ...
  methods: {
    ...
    getInList() {
      ...
      this.outFlag = false;
      fuel
        .carList(this.listQuery)
        .then((r) => {
          if (r.code === 20000) {
            this.list = r.data.socres;
